...
add_track("DW_OD", pcbnew.F_Cu, W_SIG, 5.0-1.3, 22.5-0.95, 13.0-2.8, 22.5+0.975)
add_track("DW_OC", pcbnew.F_Cu, W_SIG, 5.0-1.3, 22.5+0.95, 13.0+2.8, 22.5+0.975)
add_track("DW_CS", pcbnew.F_Cu, W_SIG, 5.0-1.3, 22.5, 5.0-0.95, 25.4)

# BAT_MINUS
add_track("BAT_MINUS", pcbnew.F_Cu, W_BAT, 5.0+0.95, 25.4, 13.0-2.8, 22.5+0.325)
...
# Detailed Silkscreen Labels
def add_silk_text(text, x, y, layer=pcbnew.F_SilkS, size=0.6, thickness=0.12):
...
# Header & Type-C
add_silk_text("USB-C 5V IN", CX, 2.0, size=0.55)
add_silk_text("TP4056 1A CHARGER", CX, 9.7, size=0.6)

# Left Side Labels (C1, C2, R3)
add_silk_text("10uF", 2.4, 10.0, size=0.45)
add_silk_text("100n", 2.4, 12.6, size=0.45)
add_silk_text("1.2k", 2.4, 15.6, size=0.45)

# Center Capacitor
add_silk_text("10uF", CX, 17.5, size=0.45)

# LED Indicators (Right side)
add_silk_text("CHRG", 13.0, 13.6, size=0.5)
add_silk_text("FULL", 13.0, 18.6, size=0.5)

# Protection Section
add_silk_text("DW01A", 5.0, 21.0, size=0.45)
add_silk_text("FS8205A", 13.0, 21.6, size=0.45)

# Battery Output
add_silk_text("BATTERY 3.7V", CX, 25.2, size=0.55)
add_silk_text("BAT +", 5.8, 27.2, size=0.65)
add_silk_text("BAT -", 12.2, 27.2, size=0.65)

# Bottom Layer (Mã số SV, Tên Đồ Án)
add_silk_text("USB-C TP4056 LI-ION CHARGER", CX, 10.0, pcbnew.B_SilkS, size=0.7)
add_silk_text("MSSV: 23207124", CX, 14.5, pcbnew.B_SilkS, size=0.8)
add_silk_text("LE NGOC TUONG", CX, 18.0, pcbnew.B_SilkS, size=0.8)
add_silk_text("HCMUT - 2026", CX, 22.0, pcbnew.B_SilkS, size=0.6)

pcbnew.SaveBoard("/mnt/Windows/HK3-25-26/KhoaHe_PCB/DoAn/USB_TypeC_TP4056_Charger/USB_TypeC_TP4056_Charger.kicad_pcb", board)
print("PCB V8 Complete with Full Annotations!")
