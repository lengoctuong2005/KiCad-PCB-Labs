...
# Board Outline: 18.0 mm x 29.0 mm
# Center X = 9.0 mm
CX = 9.0
fps = {}

# 1. J1: USB-C Connector (Rotated 0 deg, opening is at Y=0)
# Front mounting shield holes at Y=1.5, Signal pins at Y=6.3
...
# 4. Status LEDs (CHRG / FULL) & Current Limiting Resistors on Right Edge
...
fps["U2"] = add_fp("U2", "DW01A", FP_LIB, "SOT-23-6", 4.5, 22.2, 0)
fps["R6"] = add_fp("R6", "100", SMD_R[0], SMD_R[1], 2.4, 21.0, 90)
fps["C4"] = add_fp("C4", "100nF", SMD_C[0], SMD_C[1], 2.4, 23.8, 90)
fps["R7"] = add_fp("R7", "1k", SMD_R[0], SMD_R[1], 4.5, 25.4, 0)

# FS8205A Dual N-MOSFET (Q1)
fps["Q1"] = add_fp("Q1", "FS8205A", FP_LIB, "TSSOP-8_4.4x3mm_P0.65mm", 13.0, 22.5, 0)
...
# 6. DW01A Protection
add_track("DW_VCC", pcbnew.F_Cu, W_SIG, 2.4, 21.0+0.95, 4.5+1.3, 22.2)
add_track("DW_VCC", pcbnew.F_Cu, W_SIG, 2.4, 21.0+0.95, 2.4, 23.8-0.95)
add_track("DW_OD", pcbnew.F_Cu, W_SIG, 4.5-1.3, 22.2-0.95, 13.0-2.8, 22.5+0.975)
add_track("DW_OC", pcbnew.F_Cu, W_SIG, 4.5-1.3, 22.2+0.95, 13.0+2.8, 22.5+0.975)
add_track("DW_CS", pcbnew.F_Cu, W_SIG, 4.5-1.3, 22.2, 4.5-0.95, 25.4)

# 7. BAT_MINUS
add_track("BAT_MINUS", pcbnew.F_Cu, W_BAT, 4.5+0.95, 25.4, 13.0-2.8, 22.5+0.325)
