...
# Board Outline: 18.0 mm x 29.0 mm (W x H)
# Center X = 9.0 mm
CX = 9.0
fps = {}

# 1. J1: USB-C Connector
# Socket opening flush with top edge
fps["J1"] = add_fp("J1", "USB-C", FP_LIB, "USB_C_Receptacle_HRO_TYPE-C-31-M-12", CX, 2.3, 0)
...
# 4. Status LEDs & Resistors
fps["R4"] = add_fp("R4", "1k", SMD_R[0], SMD_R[1], 15.6, 11.2, 0)
fps["D1"] = add_fp("D1", "RED", SMD_LED[0], SMD_LED[1], 15.6, 13.6, 0)

fps["R5"] = add_fp("R5", "1k", SMD_R[0], SMD_R[1], 15.6, 16.2, 0)
fps["D2"] = add_fp("D2", "GREEN", SMD_LED[0], SMD_LED[1], 15.6, 18.6, 0)
...
# Symmetrically and cleanly arranged
fps["U2"] = add_fp("U2", "DW01A", FP_LIB, "SOT-23-6", 5.0, 22.5, 0)
fps["R6"] = add_fp("R6", "100", SMD_R[0], SMD_R[1], 2.4, 21.0, 90)
fps["C4"] = add_fp("C4", "100nF", SMD_C[0], SMD_C[1], 2.4, 23.8, 90)
fps["R7"] = add_fp("R7", "1k", SMD_R[0], SMD_R[1], 5.0, 25.4, 0)
...
add_track("VBUS", pcbnew.F_Cu, W_PWR, CX-0.75, 6.3, 6.5, 8.5)
add_track("VBUS", pcbnew.F_Cu, W_PWR, 6.5, 8.5, 2.4-0.95, 11.2)
add_track("VBUS", pcbnew.F_Cu, W_PWR, 2.4-0.95, 11.2, 2.4-0.95, 13.8)
add_track("VBUS", pcbnew.F_Cu, W_PWR, 2.4-0.95, 13.8, CX-2.7, 13.5+1.905)
...
# VBUS to R4, R5
add_track("VBUS", pcbnew.F_Cu, W_SIG, CX+0.75, 6.3, 15.6-0.95, 8.5)
...
add_track("CC1", pcbnew.F_Cu, W_SIG, CX-2.25, 6.3, 4.5, 7.8-0.95)
add_track("CC2", pcbnew.F_Cu, W_SIG, CX+2.25, 6.3, 13.5, 7.8-0.95)

# 3. PROG R3 -> U1 Pin 2
add_track("PROG", pcbnew.F_Cu, W_SIG, CX-2.7, 13.5-0.635, 2.4+0.95, 16.8)

# 4. LED traces
add_track("NET_R4_D1", pcbnew.F_Cu, W_SIG, 15.6+0.95, 11.2, 15.6+0.95, 13.6)
add_track("CHRG", pcbnew.F_Cu, W_SIG, 15.6-0.95, 13.6, CX+2.7, 13.5-0.635)

add_track("NET_R5_D2", pcbnew.F_Cu, W_SIG, 15.6+0.95, 16.2, 15.6+0.95, 18.6)
add_track("STDBY", pcbnew.F_Cu, W_SIG, 15.6-0.95, 18.6, CX+2.7, 13.5+0.635)
...
add_track("BAT_OUT", pcbnew.F_Cu, W_SIG, CX, 18.8-0.95, 2.4, 21.0-0.95)

# 6. DW01A Protection
add_track("DW_VCC", pcbnew.F_Cu, W_SIG, 2.4, 21.0+0.95, 5.0+1.3, 22.5)
add_track("DW_VCC", pcbnew.F_Cu, W_SIG, 2.4, 21.0+0.95, 2.4, 23.8-0.95)
add_track("DW_OD", pcbnew.F_Cu, W_SIG, 5.0-1.3, 22.5-0.95, 13.0-2.8, 23.0+0.975)
add_track("DW_OC", pcbnew.F_Cu, W_SIG, 5.0-1.3, 22.5+0.95, 13.0+2.8, 23.0+0.975)
add_track("DW_CS", pcbnew.F_Cu, W_SIG, 5.0-1.3, 22.5, 5.0-0.95, 25.4)

# 7. BAT_MINUS
add_track("BAT_MINUS", pcbnew.F_Cu, W_BAT, 5.0+0.95, 25.4, 13.0-2.8, 23.0+0.325)
add_track("BAT_MINUS", pcbnew.F_Cu, W_BAT, 13.0-2.8, 23.0+0.325, CX+1.0, 26.6)

# 8. FS8205A GND & Drain
add_track("GND", pcbnew.F_Cu, W_BAT, 13.0-2.8, 23.0-0.325, CX, 22.5)
add_via("GND", CX, 22.5)

add_track("DW_D12", pcbnew.F_Cu, W_BAT, 13.0-2.8, 23.0-0.975, 13.0+2.8, 23.0-0.975)
add_track("DW_D12", pcbnew.F_Cu, W_BAT, 13.0+2.8, 23.0-0.975, 13.0+2.8, 23.0+0.325)
...
# Professional Non-overlapping Silkscreen
def add_silk_text(text, x, y, layer=pcbnew.F_SilkS, size=0.7, thickness=0.14):
...
add_silk_text("TP4056 1A", CX, 9.8, size=0.7)
add_silk_text("CHRG", 13.0, 13.6, size=0.55)
add_silk_text("FULL", 13.0, 18.6, size=0.55)
add_silk_text("B+", 6.2, 26.6, size=0.8)
add_silk_text("B-", 11.8, 26.6, size=0.8)

add_silk_text("LNT 23207124", CX, 14.5, pcbnew.B_SilkS, size=1.0)

pcbnew.SaveBoard("/mnt/Windows/HK3-25-26/KhoaHe_PCB/DoAn/USB_TypeC_TP4056_Charger/USB_TypeC_TP4056_Charger.kicad_pcb", board)
print("Complete PCB V6!")
